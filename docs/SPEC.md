...
| テーブル | 役割 | 主なカラム |
|----------|------|-----------|
| `users` | アカウント | `id(uuid)`, `email(unique)`, `password_hash(null可)`, `username`, `handle(unique・設定後不変)`, `notifications(jsonb)`, `created_at` |
...
| GET/PATCH | `/account` | アカウント設定（メール / `username` / `handle`〔設定後不変〕 / `notifications`）の取得・更新 | 要ログイン |
| GET | `/auth/oauth/providers` | 利用可能なソーシャルログイン `{google, github}` | 公開 |
| GET | `/auth/oauth/:provider` | 認可画面へリダイレクト（state を Cookie に保存） | 公開 |
| GET | `/auth/oauth/:provider/callback` | コード交換→find/create→ログイン Cookie→`/` | 公開 |
...
### 9.2 `todo.txt` の未実装要望
- 機能: TD 一式のインポート/エクスポート。
  （※レポーティング、ワークスペース名変更、レポートの CSV 出力、**アカウント設定画面**、
  **ソーシャルログイン（Google / GitHub）** は実装済み。OAuth は各プロバイダの Client ID/Secret 設定で有効化）
- UI（現状コードに未反映）:
